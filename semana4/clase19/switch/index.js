...
function getselector() {
  ...
  let canciones = [
    ...
    `Fin Del tiempo para el que no ve Para el que duda y miente Y cree que se salvara Fe No se encuentra en lo material Es una escala espiritual Que aún nos falta conocer Libre Sólo serás con verdad Perdona y aprende a pedir perdón Eso es libertad No mires más Lo que te daña Porque no es una aventura Es tu vida real Cambiemos El mundo con unión Pensemos con el corazón La mejor forma de pensar`,
    ...
    `Tengo sed te nesecito para beber tu sangre y convertirme en parte de ti y ser algo o parte almenos porque no soy nada nos soy nada escribiendo lo vivido soy sudor y lagrimas no soy nada ante un todo que me mata quiero tu sangre porque muero porque muero`,
  ];

  switch (selector) {
    case "cancion1":
      title.textContent = "Decir adiós";
      letraCancion.textContent = canciones[0];
      break;
    case "cancion2":
      title.textContent = "Pink Floyd - Eclipse";
      letraCancion.textContent = canciones[1];
      break;
    case "cancion3":
      title.textContent = "Fin del tiempo";
      letraCancion.textContent = canciones[2];
      break;
    case "cancion4":
      title.textContent = "Last train to london - Electric Light Orchestra";
      letraCancion.textContent = canciones[3];
      break;
    case "cancion5":
      title.textContent = "Sed";
      letraCancion.textContent = canciones[4];
      break;
    default:
      title.textContent = "";
  ...
  canciones.push("elemento6");
  console.log("el ultimo elemento es ", canciones[canciones.length - 1]);
  elemento.pop()
}
